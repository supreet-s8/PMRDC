...
# Since all PNSA DC sites will start rsync at the same time, we randomly sleep from 1 to 240 secs.
...
do
  ...
  OUT="`$RSYNC -v $RSYNCOPT $BASEPATH/var root@${iREMOTEHOST}:$REMOTEDATAPATH/ 2>&1`"
  if [[ $? -eq 0 ]] ;
  then
